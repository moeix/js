/* Files App 0.2.2 - https://files.photo.gallery/ */ ! function(e) {
    function t(e) {
    ...
    function J() {
        ...
        if (!o || o != _c.qrx && O(o) != n) return _c.qrx || !n || n.includes(".") ? !_c.qrx || "string" == typeof _c.qrx && /^[a-f0-9]{32}$/.test(_c.qrx) ? (e = _c.qrx ? "key=" + _c.qrx + "&" : "", t = function(e) {
            ...
            params: e + O("YXBwPTImaG9zdD0=") + encodeURI(n),
        ...
        function l(e, t) {
            var i = Z("license_modal", 0, (e ? '<div class="alert alert-danger alert-dismissible fade show" role="alert"><button class="btn close" data-dismiss="alert">' + E.get_svg("close") + "</button><strong>Invalid license</strong><br>You have entered an invalid license key.</div>" : "") + '<div id="pay" class="mb-2"><div id="pay_info" class="mb-3 mt-1"><strong>Purchase a license [$39]</strong> to unlock features and support us!\t\t<ul class="ul-reset"><li>' + E.get_svg("check") + "Remove this popup</li><li>" + E.get_svg("check") + "Dedicated support</li><li>" + E.get_svg("check") + 'File manager, uploader, code editor, users, panorama viewer and more <strong>[coming soon]</strong></li></ul><small>* After purchase, you will receive <strong>license key</strong> by email.</small>\t\t</div><div id="pay_pp"></div></div>\t\t<form id="license_form" novalidate>\t\t\t<input id="license_key" class="form-control" type="text" placeholder="LICENSE-KEY" required>\t\t\t<div class="float-right mt-3">\t\t\t\t<button id="continue_free" type="button" class="btn btn-light" data-dismiss="modal">No thanks!</button>\t\t\t\t<button id="license_save_button" type="submit" class="btn btn-success text-light" style="display:none;">Save</button>\t\t\t</div>\t\t</form>'),
                n = _id("license_form"),
                o = n.parentNode,
                l = _id("license_key"),
                p = _id("license_save_button"),
                s = _id("continue_free"),
                c = _id("pay"),
                r = _id("pay_pp");

            function d(e) {
                o.style.cssText = e ? "pointer-events:none;opacity:.5" : ""
            }

            function m(e) {
                var t = l.value.trim(),
                    o = t && key_regex.test(t),
                    c = "submit" === e.type;
                l.classList.toggle("is-valid", o), l.classList.toggle("is-invalid", (t || c) && !o), p.style.display = o ? "" : "none", s.style.display = o ? "none" : "", c && (e.preventDefault(), e.stopPropagation(), o && (S(a, !0), d(1), q({
                    params: O("YWN0aW9uPWxpY2Vuc2Uma2V5PQ") + t,
                    json_response: !0,
                    complete: function(e, t, a) {
                        var o = a && e.success;
                        n.innerHTML = '<div class="alert alert-' + (o ? "success" : "danger") + ' mb-0" role="alert">' + E.get_svg(o ? "check" : "alert_circle_outline") + (o ? "Great success!" : "Failed to save license key") + "</div>", d(0), o && (_c.qrx = e.md5, setTimeout((function() {
                            i.hide()
                        }), 2e3))
                    }
                })))
            }
            pay_info = _id("pay_info"), key_regex = new RegExp(O("XkYxLVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9LVtBLVowLTldezR9JA==")), b(n, m, "input"), b(n, m, "submit"), t && t.Buttons({
                ...
                onError: function(e) {
                    j("error", e), c.innerHTML = '<div class="alert alert-danger" role="alert">' + E.get_svg("alert_circle_outline") + "Payment error</div>"
                },
                onApprove: function(e, t) {
                    return d(1), j("onApprove", e, t), t.order.capture().then((function(e) {
                        d(0), c.innerHTML = "<p><strong>" + e.payer.name.given_name + " " + e.payer.name.surname + "</strong><br>" + e.payer.email_address + "</p><p>" + E.get_svg("check") + "Thanks for purchasing! Please check your email.</p>", j("details", e), S(a, 1)
                    }))
                }
            }).render(r)
        }
    ...
    _c.check_updates && (S("files:updated") ? (S("files:updated", !0), Z("updated", 0, '<div data-dismiss="modal">' + E.get_svg("check") + "&nbsp; Successfully updated to Files app <strong>version " + _c.version + "</strong>.</div>")) : q({
        json_response: !0,
        params: "action=check_updates",
        complete: function(e, t, i) {
            if (e && t && i && e.hasOwnProperty("success")) {
                var a = e.success;
                if (j(a ? "New version " + a + " available." : "Already using latest version " + _c.version), a) {
                    if (!W.download && !e.writeable) return void j("Download disabled and update not writeable");
                    _id("change-sort").insertAdjacentHTML("afterend", '<div id="files-notifications" class="dropdown"><button type="button" class="btn-icon btn-topbar">' + E.get_svg("bell") + '</button><div class="dropdown-menu dropdown-menu-right" style="min-width: auto;"><h6 class="dropdown-header">Files ' + a + (e.writeable ? "" : " Update") + '</h6><div style="padding: .5rem 1rem; white-space: nowrap;">' + (W.download ? '<a href="https://cdn.jsdelivr.net/npm/files.photo.gallery@' + a + '/index.php" class="btn btn-sm btn-' + (e.writeable ? "secondary" : "primary") + '"' + (e.writeable ? "" : ' style="width:100%"') + " download>" + G("download") + "</a>" : "") + (e.writeable ? '<button type="button" class="btn btn-primary btn-sm btn-update" style="margin-left: 1px;' + (W.download ? "" : "width:100%;") + '">' + G("update") + "</button>" : "") + "</div></div></div>");
                    var n = _id("files-notifications"),
                        o = n.firstChild;
                    if (E.dropdown(n, o), e.writeable) {
                        var l = _class("btn-update", n)[0];

                        function p(e, t) {
                            l.disabled = e, l.style.opacity = e ? .8 : 1, l.innerText = G(t)
                        }
                        b(l, (function() {
                            confirm("Are you sure you want to update Files app to latest version " + a + "?") && (p(!0, "updating …"), q({
                                params: "action=do_update&version=" + a,
                                json_response: !0,
                                complete: function(e, t, i) {
                                    if (e && t && i && e.hasOwnProperty("success") && e.success) {
                                        l.innerText = G("updated!"), S("files:updated", 1);
                                        try {
                                            location.reload(!0)
                                        } catch (e) {
                                            Z("updated_refresh", 0, E.get_svg("check") + "&nbsp; Successfully updated. <strong>Please refresh browser!</strong>")
                                        }
                                    } else l.classList.add("btn-danger"), l.innerText = G("failed"), setTimeout((function() {
                                        l.classList.remove("btn-danger"), p(!1, "update")
                                    }), 3e3)
                                }
                            }))
                        }))
